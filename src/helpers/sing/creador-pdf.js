...
function checkCert (input) {
  ...
  readFile(certFile).then(readerResult => {
    ...
    console.log('Certificado Público leído')
    let certJson = '{"cert": "' + certBase64 + '"}'

    fetch('https://efirma.uaf.sspc.gob.mx/ws/validate/certificate', {
      method: 'POST',
      body: certJson
    })
      .then(resultJson => resultJson.json())
      .then(resultObject => {
        console.log(resultObject.contenido.puestoSSPC)
        var arrayDeCadenas = resultObject.contenido.puestoSSPC.split('-')
        for (var i = 0; i < arrayDeCadenas.length; i++) {
          console.log(arrayDeCadenas[i])

          if (arrayDeCadenas[i] == 'G00') {
            console.log('Si entra: ' + arrayDeCadenas[i])
            $('#oadText').val('G00')
          }
          if (arrayDeCadenas[i] == '142') {
            console.log('Si reconoce 142')
            $('#oadText').val('142')
          }
        }

        console.log('Validacion de certificado en ejecución')
        if (!resultObject.error) {
          document.getElementById('RFC').value = resultObject.contenido.rfc
          firmarBtn.disabled = false
          objeto = resultObject
          console.log('Validacion de certificado Exitosa')
        } else {
          let texto = ''
          ...
          console.log('Validacion de certificado fallida')
        }
      })
      .catch(() => {
        console.error('SIRNO no se encuentra disponible o alcanzable')
        Swal.fire({
          title: 'Aviso de proceso.',
          text: 'El sistema de RH no se encuentra disponible, favor de intentar mas tarde',
          icon: 'error'
        }).then(() => {
          $('#loaderEspera').modal('hide')
        })
      })
  })
}
...
function createPdf () {
  return new Promise(resolve => {
    let inputDocumento = document.getElementById('archivoInput')
    let documentoFile = inputDocumento.files[0]
    let inputCert = document.getElementById('crtInput')
    let certFile = inputCert.files[0]
    ...
    margen = 5
    let rutaImagen = ''
    let oadFirma = $('#oadText').val()
    if (oadFirma == 'G00') {
      rutaImagen = 'cabecera_sesnsp.png'
    }
    if (oadFirma == '142') {
      // rutaImagen = "cabecera_sesnsp.png";
      rutaImagen = 'cabecera_2024.png'
    }
    loadImage('/' + rutaImagen, function (imgCabecera) {
      readFile(documentoFile)
        .then(documento => {
          ...
          readFile(certFile)
            .then(cert => {
              ...
              doc.addImage(
                ...
                distY,
                doc.getPageWidth(),
                imgCabecera.height * (doc.getPageWidth() / imgCabecera.width)
              )
              distY =
                distY +
                imgCabecera.height * (doc.getPageWidth() / imgCabecera.width) +
                separador
              doc.setFontSize(16)
              doc.text('Firma de documento', margen, distY)
              distY = distY + separador
              doc.line(
                separador,
                distY,
                doc.getPageWidth() - separador,
                distY,
                'S'
              )
              distY = distY + separador
              doc.setFontSize(12)
              doc.text('Nombre: ' + objeto.contenido.cnData, margen, distY)
              distY = distY + separador + texto
              doc.text(
                'Correo Electrónico: ' + objeto.contenido.email,
                margen,
                distY
              )
              distY = distY + separador + texto
              doc.text('RFC: ' + objeto.contenido.rfc, margen, distY)
              distY = distY + separador + texto
              doc.text(
                'Número de empleado: ' + objeto.contenido.numEmpleadoSSPC,
                margen,
                distY
              )
              distY = distY + separador + texto
              doc.text(
                'Dependencia: ' + objeto.contenido.datosDependenciaFirmante,
                margen,
                distY
              )
              distY = distY + separador + texto
              fechaFirma = new Date(objeto.contenido.timestamp)
              fechaFormateada = fechaFirma.toLocaleDateString('es-ES', {
                weekday: 'long',
                year: 'numeric',
                month: 'long',
                day: 'numeric'
              ...
              fechaFormateada =
                ...
                fechaFirma.getHours() +
                ':' +
                fechaFirma.getMinutes() +
                ':' +
                fechaFirma.getSeconds() +
                '.' +
                fechaFirma.getMilliseconds()
              doc.text(
                'Hora de firma del documento: ' + fechaFormateada,
                margen,
                distY
              )
              distY = distY + separador + texto
              doc.text(
                'Vigencia de certificado desde: ' +
                  objeto.contenido.fechaInicial,
                margen,
                distY
              )
              distY = distY + separador + texto
              doc.text(
                'Vigencia de certificado hasta: ' + objeto.contenido.fechaFinal,
                margen,
                distY
              )
              distY = distY + separador + texto
              doc.text('Firmado utilizando FIEL', margen, distY)
              distY = distY + separador + texto
              doc.setFontSize(12)
              md5Documento = getMd5StringFromAB(documento)
              doc.text('Cadena del documento: ' + md5Documento, margen, distY)
              distY = distY + separador + texto
              doc.text('Certificado: ', margen, distY)
              distY = distY + separador
              doc.setFontSize(6)
              doc.text(
                doc.splitTextToSize(
                  getCert(cert)
                    ...
                    .replaceAll('\n', '')
                    .replaceAll('\r'),
                  doc.getPageWidth() - 2 * margen
                ),
                margen,
                distY
              )
              distY = distY + 40 + separador
              doc.setFontSize(12)
              doc.text('Validación certificado: ', margen, distY)
              distY = distY + separador
              doc.setFontSize(6)

              doc.text(
                doc.splitTextToSize(
                  objeto.contenido.ocspData,
                  doc.getPageWidth() - 2 * margen
                ),
                margen,
                distY
              )
              distY = distY + 100 - 2 * separador
              doc.setFontSize(8)
              doc.text('Vinculo para validar el documento: ', margen, distY)
              distY = distY + separador
              doc.text(
...
function procFirma () {
  // $('#loaderEspera').modal('show')
  document.getElementById('firmar').disabled = true
  const loader = document.getElementById('loader')

  loader.classList.add('container-loader')
  loader.classList.remove('hidden', 'animate__fadeOut')
  document.getElementById('loader-message').textContent =
    'Espere, Estamos firmando tu documento!'

  let inputCert = document.getElementById('crtInput')
  let inputKey = document.getElementById('keyInput')
  ...
  readFile(certFile).then(cert => {
    console.log('Cert leído')
    readFile(keyFile).then(key => {
      console.log('Key leído')
      readFile(documentoFile).then(documento => {
        console.log('Documento leído')
        let error = false
        let msgError = ''

        if (!abrirPKconPassword(key, password) && !error) {
          error = true
          msgError =
            msgError +
            ' ' +
            'El  Archivo KEY y  Contraseña de la llave privada no coinciden \n'
        }
        if (!certificadoVigente()) {
          error = true
          msgError =
            msgError + ' ' + 'El certificado no se encuentra vigente \n'
        }
        if (!keyMatchCert(cert, key, password)) {
          error = true
          msgError =
            msgError +
            ' ' +
            'El archivo de certificado y el archivo KEY no coinciden \n'
        }
        if (!verificaTipoArchivo(documentoFile)) {
          error = true
          msgError =
            msgError + ' ' + 'El documento no es admitido, debe ser PDF \n'
        }

        verificaOCSP()
          .then(objetoOcsp => {
            objeto = objetoOcsp
            createPdf().then(async pdfFirma => {
              let mergedPdf = await mergePdf(pdfFirma, documento)
              let pdfWithPlaceholder = await pdfAddPlaceholder(mergedPdf)
              let p12Buffer = await getP12Cert(cert, key, password)
              try {
                let pdfFirmado = await sign(
                  pdfWithPlaceholder,
                  password,
                  p12Buffer
                )
                ...
                window.open(url)
                document.getElementById('firmar').remove()
                const button = document.createElement('button')
                button.type = 'button'
                button.className =
                  'px-10 py-4 rounded-2xl text-white transition-all duration-500 hover:bg-blue-500 hover:scale-[1.03] bg-gradient-to-l from-[#534015] to-[#a57f2c] flex items-center gap-2'
                ...
                span.textContent = 'Firmar nuevo documento'
                ...
                setTimeout(() => {
                  Toastify({
                    text: 'Por favor verifica que tus credenciales para continuar',
                    gravity: 'top',
                    className:
                      'min-w-[400px] rounded-xl shadow-md py-5 px-10 border-red-500 border',
                    style: {
                      background:
                        'linear-gradient(82deg, rgba(74, 8, 8, 0.99) 30%, rgb(235, 0, 55) 80%, rgba(255, 4, 4, 0.92) 92%)'
                    },
                    position: 'center'
                  }).showToast()
                }, 1000)
                ...
                loader.classList.remove('container-loader')
              }
            })
          })
          .catch(() => {
            Toastify({
              text: 'En certificado no ha sido aprobado por el SAT',
              gravity: 'top',
              className:
                'min-w-[400px] rounded-xl shadow-md py-5 px-10 border-yellow-500 border',
              style: {
                background:
                  'linear-gradient(82deg, rgba(46, 38, 22, 0.99) 30%, rgb(128, 101, 20) 80%, rgba(138, 118, 6, 0.92) 92%)'
              },
              position: 'center'
            }).showToast()
          })
